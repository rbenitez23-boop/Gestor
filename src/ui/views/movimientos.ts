...
export function renderMovimientos(container: HTMLElement, db: Database, onChanged: () => void) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="padding:14px 16px;margin-bottom:16px">
      <div class="fg" style="margin-bottom:10px"><label class="fl">Buscar material o número de serie</label><input class="fc" id="mov-buscar" placeholder="Ej. 'Bocina' o 'Botiquín #3'…"/></div>
    ...
    <div class="card"><div class="tbl-wrap"><table>
      <thead><tr><th>Fecha</th><th>Material</th><th>Núm. Serie</th><th>Tipo</th><th>Unidades</th><th>Origen</th><th>Destino/Cliente</th><th></th></tr></thead>
      ...
      ? list
          .map(
            (m) => `<tr>
              ...
              <td style="font-size:12px">${esc(m.numSeries || '—')}</td>
              <td>${esc(m.tipo)}</td>
          ...
          .join('')
      : '<tr><td colspan="8" class="empty-state">Sin movimientos con estos filtros</td></tr>';
  ...
  function aplicarFiltro() {
    ...
    const q = (container.querySelector('#mov-buscar') as HTMLInputElement).value.trim().toLowerCase();
    filtrados = todos.filter((m) => {
      ...
      if (q && !m.materialNombre.toLowerCase().includes(q) && !(m.numSeries || '').toLowerCase().includes(q)) return false;
      return true;
    });
    renderRows(filtrados);
    const contador = container.querySelector('#mov-contador')!;
    contador.textContent = desde || hasta || q ? `${filtrados.length} de ${todos.length} registrados (filtrado)` : `${todos.length} registrados`;
  }

  renderRows(todos);
  container.querySelector('#mov-buscar')?.addEventListener('input', aplicarFiltro);
  ...
  container.querySelector('#mov-limpiar')?.addEventListener('click', () => {
    (container.querySelector('#mov-buscar') as HTMLInputElement).value = '';
...
function openMovimientoModal(db: Database, onChanged: () => void) {
  ...
  const body = `
    ...
    <div class="fg"><label class="fl">Número(s) de serie / distintivo (si aplica)</label><input class="fc" id="mv-serie" placeholder="Ej. Botiquín #3"/></div>
    ...
    try {
      ...
      await store.mutate((current) => {
        const { db: next, idMov } = agregarMovimiento(current, {
          ...
          fechaRegreso: '',
          numSeries: (document.getElementById('mv-serie') as HTMLInputElement).value,
        });
        idMovCreado = idMov;
        return next;
