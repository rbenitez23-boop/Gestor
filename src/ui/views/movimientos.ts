import type { Database, TipoMovimiento, TipoPaquete } from '../../types';
...
export function renderMovimientos(container: HTMLElement, db: Database, onChanged: () => void) {
  const movs = [...db.movimientos].reverse();

  container.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      <div><h1 style="font-size:22px;font-weight:800">Movimientos</h1><p style="color:var(--gris-med);font-size:13px">${db.movimientos.length} registrados</p></div>
  ...
  container.querySelector('#btn-export-mov')?.addEventListener('click', () => {
    downloadCsv(
      ...
      movs.map((m) => [m.idMov, new Date(m.fecha).toLocaleString('es-MX'), m.materialNombre, m.tipo, m.cantPaquetes, m.unidadesPaq, m.totalUnidades, m.origen, m.destino, m.cliente, m.responsable, m.notas, m.numSeries])
    );
  });
}
